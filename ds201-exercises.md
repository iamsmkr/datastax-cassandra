...
#### 1. To check the status of running Cassandra instance
```sh
$ nodetool status
...
#### 3. Load the data for the video table from a CSV file
```
...
#### 1. Create table videos_by_tag to store video data categorised by tags
```
...
#### 2. Load the data for the video table from a CSV file
```
...
#### 1. Create table videos_by_tags to incorporate desc order by added_date
```
...
#### 3. Query records ordered by date
```
...
#### 4. Query records orered by date that are older than 2013
```
